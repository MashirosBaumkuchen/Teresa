...
import com.jascal.teresa.glide.GlideRoundTransform
import com.jascal.teresa.mvp.model.bean.DiscoverBean
import com.jascal.teresa.utils.durationFormat
 ...
 * describe
 */
class VideoDetailAdapter(mContext: Context, data: ArrayList<DiscoverBean.Issue.Item>) :
        CommonAdapter<DiscoverBean.Issue.Item>(mContext, data, object : MultipleType<DiscoverBean.Issue.Item> {
            override fun getLayoutId(item: DiscoverBean.Issue.Item, position: Int): Int {
                return when {
                    position == 0 ->
...
//        textTypeface = Typeface.createFromAsset(MyApplication.context.assets, "fonts/FZLanTingHeiS-L-GB-Regular.TTF")
    ...
    // 添加视频的详细信息
    fun addData(item: DiscoverBean.Issue.Item) {
        mData.clear()
    ...
    // 添加相关推荐等数据 Item
    fun addData(item: ArrayList<DiscoverBean.Issue.Item>) {
        mData.addAll(item)
    ...
    private var mOnItemClickRelatedVideo: ((item: DiscoverBean.Issue.Item) -> Unit)? = null

    fun setOnItemDetailClick(mItemRelatedVideo: (item: DiscoverBean.Issue.Item) -> Unit) {
        this.mOnItemClickRelatedVideo = mItemRelatedVideo
    }

    // 绑定数据
    override fun bindData(holder: ViewHolder, data: DiscoverBean.Issue.Item, position: Int) {
        when {
...
//                holder.getView<TextView>(R.id.tv_text_card).typeface =textTypeface
    ...
    // 设置视频详情数据
    private fun setVideoDetailInfo(data: DiscoverBean.Issue.Item, holder: ViewHolder) {

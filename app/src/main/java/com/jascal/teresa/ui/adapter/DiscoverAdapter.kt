...
import com.jascal.teresa.ui.activity.VideoDetailActivity
 ...
 * describe
 */
class DiscoverAdapter(context: Context, data: ArrayList<DiscoverBean.Issue.Item>)
    : CommonAdapter<DiscoverBean.Issue.Item>(context, data, -1) {
    ...
    // 添加更多数据
    fun addItemData(itemList: ArrayList<DiscoverBean.Issue.Item>) {
    ...
    override fun bindData(holder: ViewHolder, data: DiscoverBean.Issue.Item, position: Int) {
        ...
        //Banner
            ITEM_TYPE_BANNER -> {
                val bannerItemData: ArrayList<DiscoverBean.Issue.Item> = mData.take(bannerItemSize).toCollection(ArrayList())
    ...
    // 加载 content item
    private fun setVideoItem(holder: ViewHolder, item: DiscoverBean.Issue.Item) {
    ...
    // 跳转到视频详情页面播放
    private fun goToVideoPlayer(activity: Activity, view: View, itemData: DiscoverBean.Issue.Item) {
